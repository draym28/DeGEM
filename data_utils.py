...
def eval_acc(y_true, y_pred):
    # y_true: (bs), y_pred: (bs, hdim)
    if not y_pred.shape == y_true.shape:
        y_pred = y_pred.argmax(dim=-1).detach()
...
@torch.no_grad()
def evaluate_detect(model, dataset_ind, dataset_ood, return_score=False):
    model.eval()

    test_ind_score = model.detect(dataset_ind, dataset_ind.splits['test'])

    if isinstance(dataset_ood, list):
        result = []
        for d in dataset_ood:
            test_ood_score = model.detect(d, d.node_idx)
            auroc, aupr, fpr, _ = get_measures(test_ind_score.cpu(), test_ood_score.cpu())
            result += [auroc] + [aupr] + [fpr]
    else:
        test_ood_score = model.detect(dataset_ood, dataset_ood.node_idx)
        auroc, aupr, fpr, _ = get_measures(test_ind_score.cpu(), test_ood_score.cpu())
        result = [auroc] + [aupr] + [fpr]

    idx = dataset_ind.splits['test']
    classify_logits = model.classify(dataset_ind, idx)
    classify_loss = F.cross_entropy(classify_logits, dataset_ind.y[idx]).item()
    acc = eval_acc(dataset_ind.y[idx], classify_logits)

    result += [acc] + [classify_loss]
    # result += [0.] + [0.]
